...
const Info = ({ id, auth, profile, dispatch }) => {
  ...
  useEffect(() => {
    if (showFollowers || showFollowing || onEdit || changePassword) {
      dispatch({ type: GLOBALTYPES.MODAL, payload: true });
    } else {
      dispatch({ type: GLOBALTYPES.MODAL, payload: false });
    }
  }, [showFollowers, showFollowing, onEdit, changePassword, dispatch]);
      ...
      {userData.map((user) => (
        <div key={user._id} className="info_container">
          ...
          <div className="info_content">
            ...
            <h6>{user.fullname} <span className="color-violet">{user.mobile}</span></h6>
            <p className="m-0">{user.address}</p>
            <h6>{user.email}</h6>
            <a
              style={{ textDecoration: "none" }}
